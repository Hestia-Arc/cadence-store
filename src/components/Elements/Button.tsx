...
export const Button = ({ children, className }: Prop) => {
  return (
    <button className={`flex justify bg-gray-600 p-2 ${className}`}>
      {children}
    </button>
  );
};
...
export const ButtonDark: React.FC<Prop2> = (props): JSX.Element => {
  return (
    <button
      ...
      className="flex justify-center items-center gap-2 text-primary-800 text-btnSm font-semibold border-solid border-b-[1.5px] border-primary-800"
    >
...
// add to cart btn
export const ButtonPrimary: FC<IPrimaryBtn> = ({
  text,
  style,
  onClick,
  disable,
  ...
  return (
    <button
      disabled={disable}
      onClick={onClick}
      className={`h-[50px] w-full ${style}  flex justify-center items-center  text-btnMd font-semibold ${
        disable ? "bg-[#55555540] text-gray-200" : "bg-gray-700 text-gray-100"
      } rounded-lg`}
    >
      <p className="">{text}</p>
    </button>
